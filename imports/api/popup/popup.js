// Adapted from Meteor Oauth package https://github.com/meteor/meteor/blob/master/packages/oauth/oauth_browser.js
...
const Popup = {
  ...
  openCenteredPopup(url, width, height) {
    ...
    if (!newwindow || !newwindow.focus) {
      ...
      const err = new Error('The login popup was blocked by your browser.');
  ...
  show(url, callback, dimensions) {
    // default dimensions that work well for Xero
    const popup = this.openCenteredPopup(
      url,
      (dimensions && dimensions.width) || 650,
      (dimensions && dimensions.height) || 440
    );
    ...
    const checkPopupOpen = setInterval(() => {
      ...
      try {
        ...
        popupClosed = popup.closed || typeof popup.closed === 'undefined';
      } catch (e) {
